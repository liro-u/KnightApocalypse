...
class Hero extends DE.GameObject {
  constructor({
    ...
    (this.axes = { x: 0, y: 0 }), (this.isActive = isActive);
    ...
    this.animation = {
      ...
      roll: {
        ...
        callback: function() {
          ...
          DE.Audio.fx.play(
            Game.hero.armorSound[
              getRandomNumber(0, Game.hero.armorSound.length - 1)
            ],
          );
        },
      },
  ...
  getPosAsVector() {
    return new DE.Vector2(this.getWorldPos().x, this.getWorldPos().y, this);
  }
  ...
  ApplyMovement() {
    if (
      (this.axes.x > 0 && this.x < this.mapBorder.x2) ||
      (this.axes.x < 0 && this.x > this.mapBorder.x1)
    ) {
  ...
  createAnimation(firstAnimation) {
    ...
    var newAnimation = new DE.SpriteRenderer({
      spriteName: currentAnim,
      x: 5,
      y: -25,
    });
    if (nextAnim !== 'none') {
  ...
  checkCollision() {
    ...
    this.shieldRef.forEach((fallingShield) => {
      var fallingShieldPosVector = new DE.Vector2(
        fallingShield.getWorldPos().x,
        fallingShield.getWorldPos().y,
        fallingShield,
      );
      if (
        fallingShieldPosVector.isInRangeFrom(
          heroPosVector,
          fallingShield.radiusCollision,
        )
      ) {
        this.setShield(true);
    ...
    this.chestRef.forEach((fallingChest) => {
      var fallingChestPosVector = new DE.Vector2(
        fallingChest.getWorldPos().x,
        fallingChest.getWorldPos().y,
        fallingChest,
      );
      if (
        fallingChestPosVector.isInRangeFrom(
          heroPosVector,
          fallingChest.radiusCollision,
        )
      ) {
    ...
    this.meteorRef.forEach((fallingMeteor) => {
      var fallingMeteorPosVector = new DE.Vector2(
        fallingMeteor.getWorldPos().x,
        fallingMeteor.getWorldPos().y,
        fallingMeteor,
      );
      if (
        fallingMeteorPosVector.isInRangeFrom(
          heroPosVector,
          fallingMeteor.radiusCollision,
        )
      ) {
        this.takeDamage();
